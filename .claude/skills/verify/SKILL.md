...
4. `dotnet test --no-build --filter "Category!=Integration"`
5. If the change touched integration-level code (network, file system, database), also run `dotnet test --no-build --filter "Category=Integration"`.

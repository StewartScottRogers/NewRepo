...
REM  for the Curl project -- no other files required.
REM
...
REM      [5/7] Clone / update repo   (into %USERPROFILE%\Curl)
...
REM  "private_protocol_compatible: no" (its wire protocol is older than this
REM  client's). Every pane/agent API call against such a server fails with
REM  protocol_mismatch, so we treat "running but not compatible" as not-ready: stop
REM  it here, then fall through to start a fresh one on the current build.
REM
REM  We match private_protocol_compatible specifically, NOT a bare "compatible: yes".
REM  findstr /C: is a substring match and the status output also carries an
REM  "endpoint_compatible:" line, so a bare pattern would match that instead and a
REM  server with a stale private protocol would sail through the check.
echo [6/7] Ensuring a herdr server is running ...
set "SRV_RUNNING="
set "SRV_COMPAT="
"%HERDR%" status server 2>nul | findstr /I /C:"status: running" >nul && set "SRV_RUNNING=1"
"%HERDR%" status server 2>nul | findstr /I /C:"private_protocol_compatible: yes" >nul && set "SRV_COMPAT=1"
...
REM  that hook as present; every other outcome (not installed, unknown, unreadable
REM  status) falls through to the install, which herdr treats as idempotent.
REM
REM  herdr words a present hook as "<name>: current (vN)" and only says "installed"
REM  in older builds, so we accept BOTH spellings -- findstr treats several /C:
REM  patterns as an OR. Each regex is "<name>:" + optional spaces + the word, so the
REM  "<name>: not installed" line does NOT satisfy it (the "not" breaks the match).
REM -----------------------------------------------------------------------------
:ensure_integration
"%HERDR%" integration status 2>nul | findstr /I /R /C:"%~1: *current" /C:"%~1: *installed" >nul
if not errorlevel 1 (
